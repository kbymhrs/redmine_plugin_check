...
5. 診断テーブルを確認します。
6. 必要に応じて **最新バージョン情報も取得する** にチェックを入れて再表示します。
7. 必要に応じて **CSV 出力** をクリックします。

最新バージョン取得は best effort です。すべての Redmine plugin が公式な配布元や release 情報を持つわけではないため、GitHub repository を検出できる plugin のみ対象になります。ネットワーク接続、GitHub API rate limit、古い Ruby 環境の TLS 設定などによって `取得不可` / `取得失敗` になる場合があります。
